...
spark = SparkSession.builder.appName("VBP-minio-read-and-process-70").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()

# Read the datatype into a DataFrame
df = spark.read.csv('s3a://itmd521/70.txt')
...
splitDF.write.parquet("s3a://vbengaluruprabhudev/70-parquet")
splitDF.write.csv("s3a://vbengaluruprabhudev/70-csv")
